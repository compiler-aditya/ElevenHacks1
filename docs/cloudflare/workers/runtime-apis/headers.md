# Source: https://developers.cloudflare.com/workers/runtime-apis/headers/

[Skip to content](https://developers.cloudflare.com/workers/runtime-apis/headers/#_top)
...
When a header name possesses multiple values, those values will be concatenated as a single, comma-delimited string value. This means that `Headers.get` will always return a string or a `null` value. This applies to all header names except for `Set-Cookie`, which requires `Headers.getAll`. This is documented below in [Differences](https://developers.cloudflare.com/workers/runtime-apis/headers/#differences).
