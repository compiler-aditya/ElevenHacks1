...
[Skip to content](https://developers.cloudflare.com/durable-objects/platform/pricing/#_top)
...
|  | Free plan | Paid plan |
| --- | --- | --- |
| Requests | 100,000 / day | 1 million / month, + $0.15/million<br> Includes HTTP requests, RPC sessions1, WebSocket messages2, and alarm invocations |
...
1 100 messages per minute comes from the fact that 100 clients
connect to each DO, and each sends 1 message per minute.

2 The example uses 1 second because each Durable Object is active for
