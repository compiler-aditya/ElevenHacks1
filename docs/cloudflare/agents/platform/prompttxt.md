# Source: https://developers.cloudflare.com/agents/platform/prompttxt/
...
\- When extending the \`Agent\` class, ensure you provide the \`Env\` and the optional state as type parameters - for example, \`class AIAgent extends Agent { ... }\`.
...
npx wrangler hyperdrive create  --connection-string="postgres://user:password@HOSTNAME\_OR\_IP\_ADDRESS:PORT/database\_name"
